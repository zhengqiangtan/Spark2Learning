...
//                // 切割原始日志(日期 用户 搜索词 城市 平台 版本)，获取城市、平台和版本
...
//        });
//
//        // 过滤出来的原始日志(日期 用户 搜索词 城市 平台 版本)，映射为(日期_搜索词, 用户)的格式
...
//
//        // 对每天每个搜索词的搜索用户，执行去重操作，获得其 uv(一个用户搜索多个相同的搜索词算一个uv)
...
//                        String date = dateKeywordUv._1.split("_" )[0];//日期
//                        String keyword = dateKeywordUv._1.split("_" )[1];//搜索
...
//
//        // 使用Spark SQL的开窗函数，统计每天搜索 uv排名前3的热点搜索词
...
//                + "WHERE rank<=3");
//
//        // 将DataFrame转换为RDD，然后映射，计算出每天的top3搜索词的搜索 uv总数
